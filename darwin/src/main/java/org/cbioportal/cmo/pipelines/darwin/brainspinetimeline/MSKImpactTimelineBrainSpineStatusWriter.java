 ...
 * @author jake
 */
public class MSKImpactTimelineBrainSpineStatusWriter implements ItemStreamWriter<MSKImpactBrainSpineCompositeTimeline>{
    ...
    public void open(ExecutionContext executionContext) throws ItemStreamException{
        stagingFile = stagingDirectory + File.separator + "data_timeline_status_caisis_gbm.txt";
        PassThroughLineAggregator aggr = new PassThroughLineAggregator();
        flatFileItemWriter.setLineAggregator(aggr);
        flatFileItemWriter.setHeaderCallback(new FlatFileHeaderCallback(){
            @Override
            public void writeHeader(Writer writer) throws IOException{
                writer.write(StringUtils.join(new MSKImpactBrainSpineTimeline().getStatusHeaders(), "\t"));
            }
        });
    ...
    public void write(List<? extends MSKImpactBrainSpineCompositeTimeline> items) throws Exception{
        ...
        for (MSKImpactBrainSpineCompositeTimeline result : items) {
            if (!result.getStatusResult().equals(MSKImpactBrainSpineCompositeTimeline.NO_RESULT)) {
