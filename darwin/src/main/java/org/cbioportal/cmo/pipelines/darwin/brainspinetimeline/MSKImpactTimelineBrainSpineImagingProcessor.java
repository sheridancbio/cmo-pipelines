 ...
 * @author jake
 */
public class MSKImpactTimelineBrainSpineImagingProcessor implements ItemProcessor<MSKImpactBrainSpineCompositeTimeline, MSKImpactBrainSpineCompositeTimeline>{
    ...
    public MSKImpactBrainSpineCompositeTimeline process(final MSKImpactBrainSpineCompositeTimeline composite) throws Exception{
        ...
        for(String field : composite.getRecord().getImagingFields()){
            recordPost.add(composite.getRecord().getClass().getMethod("get" + field).invoke(composite.getRecord()).toString());
        }
        if(recordPost.contains("IMAGING")){
            composite.setImagingResult(StringUtils.join(recordPost, "\t"));
        }
