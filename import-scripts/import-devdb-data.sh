...
(
    ...
    if [[ $DB_VERSION_FAIL -eq 0 && ${#failed_data_source_fetches[*]} -eq 0 && $CDD_ONCOTREE_RECACHE_FAIL -eq 0 ]] ; then
        ...
        IMPORT_FAIL=0
        $JAVA_BINARY_FOR_IMPORTER -Xmx32G $JAVA_IMPORTER_ARGS --update-study-data --portal devdb-portal --use-never-import --update-worksheet --notification-file "$devdb_notification_file" --oncotree-version ${ONCOTREE_VERSION_TO_USE} --transcript-overrides-source mskcc
        if [ $? -gt 0 ]; then
        ...
        $JAVA_BINARY_FOR_IMPORTER $JAVA_IMPORTER_ARGS --send-update-notification --portal devdb-portal --notification-file "$devdb_notification_file"
    fi
