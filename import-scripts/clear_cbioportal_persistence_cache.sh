...
portal_to_cache_service_basename["msk"]="eks-msk-persistence-redis"
portal_to_cache_service_basename["msk-beta"]="cbioportal-persistence-redis"
...
if [ -n "$cache_service_basename" ] && database_number_is_valid "$cache_database_number" ; then 
    $KUBECTL_BINARY $KUBECONFIG_ARG exec -t -n "default" "$cache_leader_pod_name" -- "/bin/bash" -c 'redis-cli -a $REDIS_PASSWORD -n 1 FLUSHDB'
    cache_reset_status=$?
