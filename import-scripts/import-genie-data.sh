...
#------------ DROP TABLES IN THE NON_PRODUCTION DATABASE TO MAKE SPACE FOR CLONING
    if [ $SET_DATABASE_UPDATE_RUNNING_STATE_FAIL -eq 0 ] ; then
        ...
        if ! $DROP_TABLES_FROM_MYSQL_DATABASE_SCRIPT_FILEPATH $MANAGE_DATABASE_TOOL_PROPERTIES_FILEPATH $destination_database_color ; then
            #echo "Error during dropping of tables from mysql database $destination_database_color" >&2
...
#------------ WE WILL SKIP FURTHER PROCESSING IF NO DATABASE ALTERATIONS OCCURRED. DETECT THIS.
        if [ -z $num_studies_updated ] ; then
            ...
            # TODO does this mean we also want to COPY_TABLES_FROM_MYSQL_TO_CLICKHOUSE and continue with the switch?
            ...
            if [[ $num_studies_updated != "0" ]]; then
                ...
                #CLEAR_PERSISTENCE_CACHE=1 # TODO do we still want to do this even if the clickhouse bit fails, probably not
                COPY_TABLES_FROM_MYSQL_TO_CLICKHOUSE=1
            fi
        fi

echo "COPY_TABLES_FROM_MYSQL_TO_CLICKHOUSE was set to $COPY_TABLES_FROM_MYSQL_TO_CLICKHOUSE"
        # update clickhouse
        SETUP_CLICKHOUSE_FAIL=0
...
####            CLEAR_PERSISTENCE_CACHE=1 # TODO is this where we should decide to do this?
        fi


#####ROB NOTE : PERSITENCE CACHE CLEARING WILL BE DONE MANUALLY AT FIRST. PERHAPS AUTOMATED SOON
