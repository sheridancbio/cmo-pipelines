...
if [ -z $JAVA_BINARY_FOR_IMPORTER ] | [ -z $JAVA_BINARY_FOR_DDP ] | [ -z $GIT_BINARY ] | [ -z $PORTAL_HOME ] | [ -z $MSK_MIND_DATA_HOME ] | [ -z $MSK_EXTRACT_COHORT_DATA_HOME ] ; then
...
$JAVA_BINARY_FOR_DDP $JAVA_DDP_FETCHER_ARGS -c mskextract -p $mskextract_dmp_pids_file -f diagnosis,radiation,chemotherapy,surgery,survival -o $MSK_EXTRACT_COHORT_DATA_HOME -r $MSKEXTRACT_DDP_DEMOGRAPHICS_RECORD_COUNT
if [ $? -gt 0 ] ; then
...
$JAVA_BINARY_FOR_IMPORTER -Xmx64g $JAVA_IMPORTER_ARGS --update-study-data --portal msk-mind-portal --notification-file $mskextract_notification_file --oncotree-version $ONCOTREE_VERSION_TO_USE --transcript-overrides-source mskcc --disable-redcap-export
if [ $? -gt 0 ]; then
...
bash $PORTAL_HOME/scripts/datasource-repo-cleanup.sh $PORTAL_DATA_HOME/msk-mind
$JAVA_BINARY_FOR_IMPORTER $JAVA_IMPORTER_ARGS --send-update-notification --portal msk-mind-portal --notification-file "$mskextract_notification_file"
