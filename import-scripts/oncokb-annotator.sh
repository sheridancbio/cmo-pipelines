...
(
    ...
    if [ $ONCOKB_ANNOTATION_SUCCESS -eq 1 ] ; then
        ...
        $PYTHON3_BINARY $MAF_ANNOTATOR_SCRIPT -b $ONCOKB_TOKEN -i $STAGING_MAF_FILE -o $ONCOKB_MAF_FILE -c $STAGING_SAMPLE_FILE -u $ONCOKB_URL -v $CANCER_HOTSPOTS_URL -a $PREVIOUS_ANNOTATED_MAF_ARGS
        if [ $? -ne 0 ] ; then
