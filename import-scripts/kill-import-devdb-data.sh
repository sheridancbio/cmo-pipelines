...
FLOCK_FILEPATH="/data/portal-cron/cron-lock/kill-import-devdb-data.lock"
(
    ...
    if [ -z "$PORTAL_HOME" ] || [ -z "$START_DEVDB_IMPORT_TRIGGER_FILENAME" ] || [ -z "$KILL_DEVDB_IMPORT_TRIGGER_FILENAME" ] || [ -z "$DEVDB_IMPORT_IN_PROGRESS_FILENAME" ] || [ -z "$DEVDB_IMPORT_KILLING_FILENAME" ] ; then
        echo "Error : kill-import-devdb-data.sh cannot be run without setting the PORTAL_HOME, START_DEVDB_IMPORT_TRIGGER_FILENAME, KILL_DEVDB_IMPORT_TRIGGER_FILENAME, DEVDB_IMPORT_IN_PROGRESS_FILENAME, DEVDB_IMPORT_KILLING_FILENAME environment variables. (Use automation-environment.sh)"
        exit 1
    fi

    if ! [ -f "$KILL_DEVDB_IMPORT_TRIGGER_FILENAME" ] ; then
    ...
    if [ -f "$DEVDB_IMPORT_KILLING_FILENAME" ] ; then
        echo "Warning : killing of import-devdb-data was in progress when another attempt to kill it began."
    fi
    # remove kill trigger and set status
    rm -f $KILL_DEVDB_IMPORT_TRIGGER_FILENAME
    touch $DEVDB_IMPORT_KILLING_FILENAME
    # cancel any start trigger
    rm -f $START_DEVDB_IMPORT_TRIGGER_FILENAME

    tmp="$PORTAL_HOME/tmp/kill-devdb-import"
    if ! [ -d "$tmp" ] ; then
        if ! mkdir -p "$tmp" ; then
    ...
    parent_process_line=$(grep import-devdb-data.sh "$process_list_filename" | grep -v kill-import-devdb-data.sh | grep import-devdb-data.log | head -n 1)
    if [ -z "$parent_process_line" ] ; then
        # nothing to kill ... importer is not running
        rm -f "$DEVDB_IMPORT_KILLING_FILENAME"
    exit 0
    fi
