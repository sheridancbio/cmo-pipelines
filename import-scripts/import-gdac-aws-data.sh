...
if [ $GDAC_IMPORT_FAIL -eq 0 ] ; then
    if [[ -f $gdac_aws_notifcation_file && $(wc -l < $gdac_aws_notification_file) -ne 0 ]] ; then
        email_filename=${gdac_aws_notification_file}_emailtext
        echo "The import of aws gdac studies (cmo) completed successfully." > $email_filename
        echo >> $email_filename
        echo Output generated by the cBioPortal importer: >> $email_filename
        echo -------------------------------------------- >> $email_filename
        cat $gdac_aws_notification_file >> $email_filename
        cat $email_filename | mail -s "AWS GDAC (cmo) cBioPortal import success" $email_list
        rm -f $email_filename
    fi
fi 

if [ $STATIC_GDAC_IMPORT_FAIL -eq 0 ] ; then
    if [[ -f $static_gdac_aws_notifcation_file && $(wc -l < $static_gdac_aws_notification_file) -ne 0 ]] ; then
        email_filename=${static_gdac_aws_notification_file}_emailtext
        echo "The import of aws gdac studies (dmp/pdx) completed successfully." > $email_filename
        echo >> $email_filename
        echo Output generated by the cBioPortal importer: >> $email_filename
        echo -------------------------------------------- >> $email_filename
        cat $static_gdac_aws_notification_file >> $email_filename
        cat $email_filename | mail -s "AWS GDAC (dmp/pdx) cBioPortal import success" $email_list
        rm -f $email_filename
    fi
fi
