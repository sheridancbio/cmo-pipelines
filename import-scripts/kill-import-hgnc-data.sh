...
FLOCK_FILEPATH="/data/portal-cron/cron-lock/kill-import-hgnc-data.lock"
(
    ...
    if [ -z "$PORTAL_HOME" ] || [ -z "$START_HGNC_IMPORT_TRIGGER_FILENAME" ] || [ -z "$KILL_HGNC_IMPORT_TRIGGER_FILENAME" ] || [ -z "$HGNC_IMPORT_IN_PROGRESS_FILENAME" ] || [ -z "$HGNC_IMPORT_KILLING_FILENAME" ] ; then
        echo "Error : kill-import-hgnc-data.sh cannot be run without setting the PORTAL_HOME, START_HGNC_IMPORT_TRIGGER_FILENAME, KILL_HGNC_IMPORT_TRIGGER_FILENAME, HGNC_IMPORT_IN_PROGRESS_FILENAME, HGNC_IMPORT_KILLING_FILENAME environment variables. (Use automation-environment.sh)"
        exit 1
    fi

    if ! [ -f "$KILL_HGNC_IMPORT_TRIGGER_FILENAME" ] ; then
    ...
    if [ -f "$HGNC_IMPORT_KILLING_FILENAME" ] ; then
        echo "Warning : killing of import-hgnc-data was in progress when another attempt to kill it began."
    fi
    # remove kill trigger and set status
    rm -f $KILL_HGNC_IMPORT_TRIGGER_FILENAME
    touch $HGNC_IMPORT_KILLING_FILENAME
    # cancel any start trigger
    rm -f $START_HGNC_IMPORT_TRIGGER_FILENAME

    tmp="$PORTAL_HOME/tmp/kill-hgnc-import"
    if ! [ -d "$tmp" ] ; then
        if ! mkdir -p "$tmp" ; then
    ...
    parent_process_line=$(grep import-hgnc-data.sh "$process_list_filename" | grep -v kill-import-hgnc-data.sh | grep import-hgnc-data.log | head -n 1)
    if [ -z "$parent_process_line" ] ; then
        # nothing to kill ... importer is not running
        rm -f "$HGNC_IMPORT_KILLING_FILENAME"
    exit 0
    fi
