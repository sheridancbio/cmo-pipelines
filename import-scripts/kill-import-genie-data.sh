...
FLOCK_FILEPATH="/data/portal-cron/cron-lock/kill-import-genie-data.lock"
(
    ...
    if [ -z "$PORTAL_HOME" ] || [ -z "$START_GENIE_IMPORT_TRIGGER_FILENAME" ] || [ -z "$KILL_GENIE_IMPORT_TRIGGER_FILENAME" ] || [ -z "$GENIE_IMPORT_IN_PROGRESS_FILENAME" ] || [ -z "$GENIE_IMPORT_KILLING_FILENAME" ] ; then
        echo "Error : kill-import-genie-data.sh cannot be run without setting the PORTAL_HOME, START_GENIE_IMPORT_TRIGGER_FILENAME, KILL_GENIE_IMPORT_TRIGGER_FILENAME, GENIE_IMPORT_IN_PROGRESS_FILENAME, GENIE_IMPORT_KILLING_FILENAME environment variables. (Use automation-environment.sh)"
        exit 1
    fi

    if ! [ -f "$KILL_GENIE_IMPORT_TRIGGER_FILENAME" ] ; then
    ...
    if [ -f "$GENIE_IMPORT_KILLING_FILENAME" ] ; then
        echo "Warning : killing of import-genie-data was in progress when another attempt to kill it began."
    fi
    # remove kill trigger and set status
    rm -f $KILL_GENIE_IMPORT_TRIGGER_FILENAME
    touch $GENIE_IMPORT_KILLING_FILENAME
    # cancel any start trigger
    rm -f $START_GENIE_IMPORT_TRIGGER_FILENAME

    tmp="$PORTAL_HOME/tmp/kill-genie-import"
    if ! [ -d "$tmp" ] ; then
        if ! mkdir -p "$tmp" ; then
    ...
    parent_process_line=$(grep import-genie-data.sh "$process_list_filename" | grep -v kill-import-genie-data.sh | grep import-genie-data.log | head -n 1)
    if [ -z "$parent_process_line" ] ; then
        # nothing to kill ... importer is not running
        rm -f "$GENIE_IMPORT_KILLING_FILENAME"
    exit 0
    fi
